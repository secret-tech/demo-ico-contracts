...
contract('DemoToken', function(accounts) {
  it("should put 2000000000 SPACE to supply and in the first account", async function () {
    const instance = await DemoToken.new();
  ...
  it("should allow transfer when token is released", async function() {
    ...
    assert.equal(balance0.valueOf(), 1999999900 * 10 ** 18);
  ...
  it("should allow transfer when token is released - fractional value", async function() {
    ...
    const balance0 = await token.balanceOf(accounts[0]);
    assert.equal(balance0.valueOf(), 1999999999.9999 * 10 ** 18);
  ...
  it("should allow to burn by owner", async function() {
    ...
    assert.equal(balance.valueOf(), 1999000000 * 10 ** 18);

    const supply = await token.totalSupply().valueOf();
    assert.equal(supply.valueOf(), 1999000000 * 10 ** 18);
  });
  ...
  it("should not allow to burn more than balance", async function() {
    ...
    try {
      await token.burn(2000000001 * 10 ** 18);
    } catch (error) {
      return assertJump(error);
    }
    assert.fail('should have thrown before');
  ...
  it("should allow to burn from by owner", async function() {
    ...
    const supply = await token.totalSupply().valueOf();
    assert.equal(supply.valueOf(), 1999500000 * 10 ** 18);
